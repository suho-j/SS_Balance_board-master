...
namespace BodyType
{
    public partial class MainForm : Form
    {
        ...
        //private UIntPtr info;

        private const int WM_KEYDOWN = 0x100; //키보드 눌릴때
        private const int WM_KEYUP = 0x101; // 키보드 땔 때
        private const int VK_RETURN = 0x0D; //Enter키
        ...
        #endregion
        private float initial1, initial2, initial3, initial4;
        private bool init;
        private bool Keyboard_Boolean;
        #region field bool
        /// <summary>
        ...
        private float GetWeight() // 몸무게 초기화
        {
            float weight = (w1 + w2 + w3 + w4);
            if ((weight > 10)&& !init)
                init = true;
            if (zedCnt == 0||(init && (weight<1)))
            {
                initial1 = w1;
                initial2 = w2;
                initial3 = w3;
        ...
        private void drawBodyType(string readLine)
        {
            ...
            try
            {
                w1 = float.Parse(ary[2]);
                w2 = float.Parse(ary[3]);
                w3 = float.Parse(ary[0]);
                w4 = float.Parse(ary[1]);

                weight = GetWeight();
                if(Keyboard_Boolean == true)
                    KeyboardHooking();
            }
            catch (Exception) {}
        ...
        {
            if (Keyboard_Boolean == false)
                Keyboard_Boolean = true;
            else
                Keyboard_Boolean = false;
        }
